...
# Additional constraints for balanced distribution across slots
for slot in slots:
    slot_clauses = [var_map[(session, slot, papers)] for session in conference_sessions for papers in papers_range]
    # Ensure each slot has at least a minimum number of sessions
    min_sessions_per_slot = len(conference_sessions) // len(slots)
    cnf.append(slot_clauses[:min_sessions_per_slot])

# Ensure maximum parallel sessions limit
for slot in slots:
    for comb in itertools.combinations(slot_clauses, max_parallel_sessions + 1):
        cnf.append([-c for c in comb])
